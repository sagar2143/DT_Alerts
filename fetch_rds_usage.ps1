...
$url = "https://etq84528.live.dynatrace.com/api/v2/metrics/query?metricSelector=(ext:cloud.aws.rds.cpuUtilization:filter(and(or(in(%22dt.entity.custom_device%22,entitySelector(%22type(custom_device),entityName.contains(~%22amstack-prod01-eu-prod-~%22)%22))))):splitBy(%22dt.entity.custom_device%22):sort(value(auto,descending))):limit(100):names:fold(max)&from=-5m&to=now&mzSelector=mzId(-1680102141339355738)"
